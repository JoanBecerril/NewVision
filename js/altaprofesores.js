function altaProfesor() {
    ...
    if (dni.trim() === "" || nombre.trim() === "" || apellido1.trim() === "" || apellido2.trim() === "" || email.trim() === "" || telefono.trim() === "" || dept_prof.trim() === ""|| sal_prof.trim() === "" ) {
    ...
    if (!salarioRegex.test(sal_prof)) {
        alert("El salario no tiene un formato válido. Debe ser un número con 2 decimales como máximo.");
        return false; // Evita que se envíe el formulario
    }
    ...
    if (isNaN(departamentoNum) || departamentoNum > 3) {
        alert("El número de departamento no es válido. Debe ser un número del 1 al 3.");
        return false; // Evita que se envíe el formulario
    }
    // Si se pasa todas las validaciones, se puede enviar el formulario
...
form.addEventListener("submit", altaProfesor);
